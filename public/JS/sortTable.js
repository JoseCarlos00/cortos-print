...
export function ordenarPorPiso2(rows, table, headerPositionElement) {
...
export function ordenarPorPiso(rows, table, headerPositionElement) {
	return new Promise((resolve, reject) => {
		try {
			...
			const hasWarehouseSomeRow = rows.some((row) =>
				row.querySelector(`td:nth-child(${headerPositionElement})`)?.innerText?.includes("W-Mar Bodega")
			);

			if (hasWarehouseSomeRow) {
				ordenarPorPiso2(rows, table, headerPositionElement);
				return;
			}
